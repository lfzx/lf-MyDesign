...
namespace PostMatch.Api.Controllers
{
    ...
    public class UserController : ControllerApiBase
    {
        ...
        [AllowAnonymous]
        [HttpPut("password/{id}")]
        public IActionResult EditPassword(string id,[FromBody]UserModel userModel)
        { 
            // map dto to entity and set id
            var user = _iMapper.Map<User>(userModel);
            user.Id = id;

            var result = _iUserService.Authenticate(userModel.Email, userModel.Password);
            ...
            if (result != null)
            {
                ...
                {
                    // save 
                    _iUserService.EditPassword(user, userModel.NewPassword);
                    var count = 1;
